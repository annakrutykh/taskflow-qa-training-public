...
firstName, lastName, email, role` — без `isActive`, это не замена
административного списка (`role` нужен, чтобы UI мог заранее показать, что
добавление этого пользователя в проект форсирует роль `ADMIN`, см. раздел
3.1). `q` обязателен (мин. 1 символ), `limit` по
