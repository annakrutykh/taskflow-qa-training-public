...
> Главный источник истины по контракту, ролям, кодам ошибок и схеме БД.
...
>
> Версия: v1 (по состоянию на 2026-07-28).
...
- Префикс всех ручек: `/api/v1`, задаётся один раз через `APIRouter(prefix="/api/v1/...")`,
  без дублирования в декораторах эндпоинтов.
...
Одновременно активных `ADMIN` не может быть больше 3 — это тоже инвариант API (`MAX_ADMINS`,
`409`): повышение роли до `ADMIN` отклоняется, если лимит уже достигнут.
...
Модуль `app/permissions/` — единая точка правды для всех проверок доступа.
...
| Списки | обёртка `{ "items": [...], "total": N, "limit": N, "offset": N, "hasNext": bool }` (`Page[T]` в `app/schemas.py`) |
| Даты | ISO 8601, UTC, суффикс `Z` |
| Enum | строки в верхнем регистре (`TODO`, `HIGH`, `ADMIN`, ...) |
| Сортировка списков | всегда вторичный ключ `id ASC` для стабильности пагинации |
...
Единый формат тела ошибки (глобальным обработчиком исключений):
...
`code` — стабильный машиночитаемый идентификатор (см. реестр ниже), не меняется. `message` —
человекочитаемый текст **на русском языке**, предназначен для отображения пользователю
как есть; ориентируйтесь на `code`, а не на точный текст `message`, если пишете тест на
конкретную ошибку. Исключение — `details` при `VALIDATION_ERROR`: это список ошибок
валидации отдельных полей от Pydantic, их текст (`msg`) может быть на английском (сообщения
самого Pydantic) — на клиенте пользователю это поле напрямую не показывается.
...
| `EMAIL_ALREADY_EXISTS` | 409 | Регистрация с уже существующим email (сравнение регистронезависимо) |
...
| `MAX_ADMINS` | 409 | Попытка повысить пользователя до `ADMIN`, когда активных `ADMIN` уже 3 |
| `TOO_MANY_REQUESTS` | 429 | Превышен лимит запросов на `/auth/login`/`/auth/register` |
| `INTERNAL_ERROR` | 500 | Необработанное исключение |

Новый код ошибки не вводится без добавления записи сюда.
...
| PATCH | `/users/{id}/role` | `ADMIN` | `200 UserResponse` | `401`, `403`, `404`, `422`, `409 LAST_ADMIN` (разжалование последнего активного ADMIN), `409 MAX_ADMINS` (повышение сверх лимита в 3 активных ADMIN) |
...
найти коллегу по имени при добавлении в проект или назначении исполнителя
задачи, не имея доступа к административному списку пользователей.
`UserSearchResult`: `id, firstName, lastName, email, role` — без `isActive`,
это не замена административного списка (`role` нужен, чтобы UI мог заранее
показать, что добавление этого пользователя в проект форсирует роль `ADMIN`,
см. раздел 3.1). `q` обязателен (мин. 1 символ), `limit` по умолчанию 20
(макс. 50).
...
`tasks.project_id`, `tasks.assignee_id`) — обязательны; добавляются миграциями Alembic.
...
Не часть курса — команды запуска, если понадобятся, в `docs/TESTING.md`.
Elasticsearch тестам не нужен
...
- `API_DOCUMENTATION.md` — документация по API для студентов, описывает поведение «как есть».
